...
- `.claude/skills/tailwind-check/SKILL.md`: 신규 스킬 추가. Tailwind CSS 컨벤션 준수 여부를 점검하는 읽기 전용 리포트 스킬. 설정 파일, CSS 파일 현황, Tailwind 클래스 사용 비율을 분석하여 판정 결과를 대화창에 출력한다.
- `CLAUDE.md`: 디렉토리 구조 섹션에 `tailwind-check/SKILL.md` 항목 추가 및 `issues.md` → `create-issue/SKILL.md`로 경로 수정. 하단에 `/tailwind-check` 실행 안내 문구 추가.
