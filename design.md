...
- **모든 크기 값은 `rem`을 사용한다.** 폰트 크기, 여백(padding/margin), 테두리 반경(border-radius), 레이아웃 치수(width, height, max-width) 모두 해당한다.
- **기준**: `1rem = 16px` (브라우저 기본값)
  - 빠른 환산 → `px ÷ 16 = rem` (예: `24px → 1.5rem`, `48px → 3rem`)
- **예외적으로 `px`를 유지하는 경우**
  - 보더 두께: `1px solid ...` — 물리적 1픽셀 선명도 유지
  - 그림자 오프셋/블러: 미세 소수점 값(`0.175px`, `2.025px` 등)은 px 유지
  - 레터스페이싱(`letter-spacing`): 타이포그래피 관례상 px 또는 em 사용
  - `9999px` pill radius: CSS 트릭값, 변환 불필요
- **미디어 쿼리 breakpoint도 rem**으로 작성한다 (예: `48rem` = 768px).
...
The custom NotionInter font (a modified Inter) is the backbone of the system. At display sizes (4rem), it uses aggressive negative letter-spacing (-2.125px), creating headlines that feel compressed and precise. The weight range is broader than typical systems: 400 for body, 500 for UI elements, 600 for semi-bold labels, and 700 for display headings. OpenType features `"lnum"` (lining numerals) and `"locl"` (localized forms) are enabled on larger text, adding typographic sophistication that rewards close reading.
...
**Key Characteristics:**
- NotionInter (modified Inter) with negative letter-spacing at display sizes (-2.125px at 4rem)
...
- 0.5rem base spacing unit with an organic, non-rigid scale
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | NotionInter | 4rem | 700 | 1.00 (tight) | -2.125px | Maximum compression, billboard headlines |
| Display Secondary | NotionInter | 3.375rem | 700 | 1.04 (tight) | -1.875px | Secondary hero, feature headlines |
| Section Heading | NotionInter | 3rem | 700 | 1.00 (tight) | -1.5px | Feature section titles, with `"lnum"` |
| Sub-heading Large | NotionInter | 2.5rem | 700 | 1.50 | normal | Card headings, feature sub-sections |
| Sub-heading | NotionInter | 1.625rem | 700 | 1.23 (tight) | -0.625px | Section sub-titles, content headers |
| Card Title | NotionInter | 1.375rem | 700 | 1.27 (tight) | -0.25px | Feature cards, list titles |
| Body Large | NotionInter | 1.25rem | 600 | 1.40 | -0.125px | Introductions, feature descriptions |
| Body | NotionInter | 1rem | 400 | 1.50 | normal | Standard reading text |
| Body Medium | NotionInter | 1rem | 500 | 1.50 | normal | Navigation, emphasized UI text |
| Body Semibold | NotionInter | 1rem | 600 | 1.50 | normal | Strong labels, active states |
| Body Bold | NotionInter | 1rem | 700 | 1.50 | normal | Headlines at body size |
| Nav / Button | NotionInter | 0.9375rem | 600 | 1.33 | normal | Navigation links, button text |
| Caption | NotionInter | 0.875rem | 500 | 1.43 | normal | Metadata, secondary labels |
| Caption Light | NotionInter | 0.875rem | 400 | 1.43 | normal | Body captions, descriptions |
| Badge | NotionInter | 0.75rem | 600 | 1.33 | 0.125px | Pill badges, tags, status labels |
| Micro Label | NotionInter | 0.75rem | 400 | 1.33 | 0.125px | Small metadata, timestamps |

### Principles
- **Compression at scale**: NotionInter at display sizes uses -2.125px letter-spacing at 4rem, progressively relaxing to -0.625px at 1.625rem and normal at 1rem. The compression creates density at headlines while maintaining readability at body sizes.
...
- **Warm scaling**: Line height tightens as size increases -- 1.50 at body (1rem), 1.23-1.27 at sub-headings, 1.00-1.04 at display. This creates denser, more impactful headlines.
- **Badge micro-tracking**: The 0.75rem badge text uses positive letter-spacing (0.125px) -- the only positive tracking in the system, creating wider, more legible small text.
...
- Radius: 0.75rem (standard cards), 1rem (featured/hero cards)
...
- Image cards: 0.75rem top radius, image fills top half
...
- Brand logo left-aligned (2.0625rem x 2.125rem icon + wordmark)
- Links: NotionInter 0.9375rem weight 500-600, near-black text
...
- Top-rounded images: `0.75rem 0.75rem 0 0` radius
...
- 0.75rem radius card with whisper border
- Title at 1.375rem weight 700, description at 1rem weight 400
...
- NotionInter 2.5rem+ weight 700 for the metric
...
### Spacing System
- Base unit: 0.5rem
- Scale: 0.125rem, 0.1875rem, 0.25rem, 0.3125rem, 0.375rem, 0.4375rem, 0.5rem, 0.6875rem, 0.75rem, 0.875rem, 1rem, 1.5rem, 2rem
- Non-rigid organic scale with fractional values (0.35rem, 0.4rem) for micro-adjustments

### Grid & Container
- Max content width: approximately 75rem
- Hero: centered single-column with generous top padding (5rem–7.5rem)
...
- **Generous vertical rhythm**: 4rem–7.5rem between major sections. Notion lets content breathe with vast vertical padding.
...
### Border Radius Scale
- Micro (0.25rem): Buttons, inputs, functional interactive elements
- Subtle (0.3125rem): Links, list items, menu items
- Standard (0.5rem): Small cards, containers, inline elements
- Comfortable (0.75rem): Standard cards, feature containers, image tops
- Large (1rem): Hero cards, featured content, promotional blocks
...
|------|-------|-------------|
| Mobile Small | <25rem | Tight single column, minimal padding |
| Mobile | 25rem–37.5rem | Standard mobile, stacked layout |
| Tablet Small | 37.5rem–48rem | 2-column grids begin |
| Tablet | 48rem–67.5rem | Full card grids, expanded padding |
| Desktop Small | 67.5rem–75rem | Standard desktop layout |
| Desktop | 75rem–90rem | Full layout, maximum content width |
| Large Desktop | >90rem | Centered, generous margins |

### Touch Targets
- Buttons use comfortable padding (0.5rem–1rem vertical)
- Navigation links at 0.9375rem with adequate spacing
- Pill badges have 0.5rem horizontal padding for tap targets
- Mobile menu toggle uses standard hamburger button

### Collapsing Strategy
- Hero: 4rem display -> scales to 2.5rem -> 1.625rem on mobile, maintains proportional letter-spacing
...
- Section spacing: 5rem+ -> 3rem on mobile
...
- "Create a hero section on white background. Headline at 4rem NotionInter weight 700, line-height 1.00, letter-spacing -2.125px, color rgba(0,0,0,0.95). Subtitle at 1.25rem weight 600, line-height 1.40, color #615d59. Blue CTA button (#0075de, 0.25rem radius, 0.5rem 1rem padding, white text) and ghost button (transparent bg, near-black text, underline on hover)."
- "Design a card: white background, 1px solid rgba(0,0,0,0.1) border, 0.75rem radius. Use shadow stack: rgba(0,0,0,0.04) 0px 4px 18px, rgba(0,0,0,0.027) 0px 2.025px 7.85px, rgba(0,0,0,0.02) 0px 0.8px 2.93px, rgba(0,0,0,0.01) 0px 0.175px 1.04px. Title at 1.375rem NotionInter weight 700, letter-spacing -0.25px. Body at 1rem weight 400, color #615d59."
- "Build a pill badge: #f2f9ff background, #097fe8 text, 9999px radius, 0.25rem 0.5rem padding, 0.75rem NotionInter weight 600, letter-spacing 0.125px."
- "Create navigation: white header. NotionInter 0.9375rem weight 600 for links, near-black text. Blue pill CTA 'Get Notion free' right-aligned (#0075de bg, white text, 0.25rem radius)."
- "Design an alternating section layout: white sections alternate with warm white (#f6f5f4) sections. Each section has 4rem–5rem vertical padding, max-width 75rem centered. Section heading at 3rem weight 700, line-height 1.00, letter-spacing -1.5px."

### Iteration Guide
1. Always use warm neutrals -- Notion's grays have yellow-brown undertones (#f6f5f4, #31302e, #615d59, #a39e98), never blue-gray
2. Letter-spacing scales with font size: -2.125px at 4rem, -1.875px at 3.375rem, -0.625px at 1.625rem, normal at 1rem
...
7. Pill badges (9999px) for status/tags, 0.25rem radius for buttons and inputs
